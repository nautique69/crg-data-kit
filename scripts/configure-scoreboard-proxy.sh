...
cat > /etc/nginx/conf.d/scoreboard.conf << EOF
stream {
        server {
                listen  80;
...
cat > /etc/nginx/conf.d/scoreboard.conf.toggle << EOF
stream {
        server {
                listen  80;
                proxy_pass      scoreboard2:80;
...
exho "/home/pi/run-proxy.sh" >> /home/pi/.config/lxsession/LXDE-pi/autostart
